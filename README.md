...
[代码](./Exercise/63/7/demo.c)

当maxevents比较小(指申请用来返回结果的evlist数组不够大)的情况下，而多个(比maxevents多)监测的fd都处于就绪态，一次epoll_wait()返回不了的情况下，那么evlist返回的结果不是固定的也不是随机的，而是轮询遍历返回所有就绪的fd，这样就让每个就绪的fd都有机会获得处理，这样避免了饥饿的发生。
