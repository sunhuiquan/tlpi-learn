...
### 18.9
fchdir()在重复调用的情况下速度快，因为chdir首先需要在用户态和内核态传递数据即路径名字符串，注意这可不是函数传参传个指针这样,首先系统调用要陷入内核态，在内核态是内核页表，显然访问不到用户态的虚拟内存，所以会传递数据(这样的参数传递数据就是拷贝移动，不过对于大量数据传输的情况下，可以内存映射实现内核空间和用户空间的共享内存完成IPC)；另外chdir需要解析路径名得到inode，虽然有高速缓存，但显然也要多做一些工作。
