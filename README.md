...
### 26.4
sleep确实是不安全的,因为如果在高负载或者特殊情况下，是有可能到了时间另外的进程也没执行完的。(另外就是sleep这个方式实际生产中也根本没用，因为休眠太浪费资源了，时间不好把控，而且还不安全)
[代码](./Exercise/26_4.c)有点意思的就是通过全局变量标志来忽略其他信号，只有SIGCHLD信号到达才退出循环往下运行。
